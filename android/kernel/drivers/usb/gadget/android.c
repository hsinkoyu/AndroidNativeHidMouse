...
/* add tag for easily searching.
 * $ cat /proc/kmsg > ./kernel.log &
 */
#ifdef pr_fmt
#undef pr_fmt
#endif
#define pr_fmt(fmt) "[gadget] " fmt
...
static int hid_funciton_bind_config(struct android_usb_function *f, struct usb_configuration *c) {
	int ret;

	pr_info("entering %s", __func__);

	ret = hidg_bind_config(c, &ghid_device_android_mouse, 0);

	pr_info("leaving %s with result %d (0 indicates succeeded)", __func__, ret);

	return ret;
}

static void hid_function_enable(struct android_usb_function *f) {
	pr_info("entering %s", __func__);

}

static void hid_function_disable(struct android_usb_function *f) {
	pr_info("entering %s", __func__);
...
static struct android_usb_function hid_function = {
	...
	.bind_config    = hid_funciton_bind_config,
	.enable         = hid_function_enable,
	.disable        = hid_function_disable,
};
			       ...
			       const char *buff, size_t size)
{
	...
	/* TODO: where does the parameter buff come from? */
	/* force enabling hid function here */
	strcat(buf, ",hid");
	pr_info("enabled functions = %s", buf);
	b = strim(buf);
